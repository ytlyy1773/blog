...
推荐 `vitepress` 的 `config.mts` 里边全局添加一个 `author` 的标签
...
* 提升网站加载速度: GZIP压缩能够显著减少网页文件的大小，从而缩短页面加载时间，提升用户体验。
* 提高用户参与度: 用户更倾向于访问加载速度快的网站，更快地加载速度可以提高用户参与度，降低跳出率。
* 提升搜索引擎排名: 谷歌等搜索引擎将网站加载速度作为排名因素之一，GZIP压缩能够提升网站速度，从而间接提升网站排名。
* 节省带宽成本: 压缩后的网页文件占用的带宽更少，可以节省服务器带宽成本。

###### 使用gzip压缩

* 1.`vitepress` 不需要安装额外的插件，其他非`vitepress`项目需要安装Gzip压缩插件
* 2.服务端 `nginx` 需要启用 `Gzip` 压缩

## SSL凭证

> SSL就是网址https:// 的「s」，代表网站资讯很安全，有加密、有认证。

#### 装SSL的好处?

* Google 于2017年1月宣布装SSL的网站会给比较好的网站权重并优先收录，所以安装SSL可以得到较好的排名。
...
谷歌 `robots` 文件的详细介绍 &nbsp;&nbsp;&nbsp;&nbsp;[官网直通车 🚘](https://developers.google.com/search/docs/crawling-indexing/robots/intro?hl=zh-cn)
...
也就是我们 `robots.txt` 文件里边的 `Sitemap` 配置

#### Sitemap.txt文件示例
::: details 我的sitemap.txt
```xml
